...
| `scripts/db-chokepoint-guard.mjs` | ADR-032: a `db_*` command reaches Tauri only through `dbStorage.invokeDb`, the single funnel that stamps the workspace epoch — so every write carries the identity of the workspace it was issued for, including the unawaited `db*(...).catch(() => {})` ones. Also asserts the chokepoint still stamps at all. Mechanical floor only: whether a DEFERRED caller presented the epoch its work was QUEUED for stays a review concern. |
