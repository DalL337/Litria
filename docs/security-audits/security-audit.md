...
| 2026-09-14 | Owner-supplied six-pass SQLite audit (durability focus, not a security pass) + same-day remediation | `src-tauri/src/db/` (schema, migrations, bootstrap, open path, error classification), `.litria/` git exposure | Report: `docs/audits/sqlite-schema-2026-09-14.md`. 0 Tier-1, 0 injection paths; 3 Tier-2 durability defects + 6 hardening items → **issue 29**, ADR-026, brief `docs/plans/persistence/brief-workspace-database-durability.md`. Fixed on `fix/adr-026-workspace-db-durability`: per-step transactional migrations that heal half-migrated files, bootstrap-target rule for a missing `project` row, `quick_check` + zero-length refusal + `db.corrupt` recovery message, result-code error classification (`db.read_only`, `db.busy`), `.gitignore` seeded when `.git` exists, `file:` database paths refused. Verification finding: the bundled SQLite is compiled with `SQLITE_USE_URI`, so the audit's proposed `SQLITE_OPEN_URI` removal is inert on its own. Slice 4 (read-only pill + persistence notices) awaits two owner rulings. |

> **Addendum (2026-09-15, ADR-027 documentation slice):** The 2026-09-14
> row's final sentence is superseded. Both owner rulings were accepted and
> slice 4 merged as PR #37 in v1.0.6.
...
drawn from fixed literals). Tier 2: S1 migrations autocommitted statement by
statement, so a crash mid-step left `duplicate column name` on every later
open; S2 a death between creating `workspace.db` and inserting the `project`
row left the project unopenable; S3 a read-only file opened silently and
every write was swallowed by `console.warn`. Tier 3 hardening H1–H6
(gitignore only appended, no `quick_check`, zero-length file re-initialized,
URI-parsed filenames, double project row, fixture drift). Decisions:
ADR-026; design and delivery notes:
`docs/plans/persistence/brief-workspace-database-durability.md`. Finding for
the next pass: rusqlite's bundled SQLite is compiled with `SQLITE_USE_URI`,
so URI parsing is global and `SQLITE_OPEN_URI` is inert — `db::open_connection`
refuses `file:`-prefixed paths instead. ⚠ The audit report sits at
`docs/audits/` by owner instruction; security-policy Rule 3 names
`docs/security-audits/` — unreconciled, owner to rule.
