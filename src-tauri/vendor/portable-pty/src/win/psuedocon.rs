...
impl PsuedoCon {
    pub fn new(size: COORD, input: FileDescriptor, output: FileDescriptor) -> Result<Self, Error> {
        ...
        // LITRIA PATCH (the sole change in this vendored copy): upstream
        // hardcodes INHERIT_CURSOR | RESIZE_QUIRK | WIN32_INPUT_MODE, which
        // fits wezterm — a terminal that answers cursor queries in-process
        // and speaks win32-input-mode. Litria's terminal is xterm.js in a
        // webview: INHERIT_CURSOR makes the host block startup on an ESC[6n
        // handshake answered too late (prompt desync), and WIN32_INPUT_MODE
        // makes the host synthesize key events from every plain VT input
        // char (sluggish typing) for a protocol xterm.js never speaks.
        // RESIZE_QUIRK is kept.
        let result = unsafe {
