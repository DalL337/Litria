/**
 * pythonLspProviders.js — Monaco language provider registrations backed by
 * the generic LSP transport (lspRequest).
 *
 * Providers are registered once via registerPythonLspProviders() called from
 * EditorMonaco's onMount.  The active session context (projectId,
 * projectRootPath) is kept in a module-level singleton updated by
 * setLspProviderContext() whenever props change in EditorMonaco.
 *
 * Covered in this file:
 *   textDocument/completion    — full item list with kind mapping
 ...
 *
 * textDocument/definition requires CM file-open integration and is deferred.
 */

import { lspRequest } from './lspClient.js';

// ---------------------------------------------------------------------------
// Module-level context (updated from EditorMonaco via setLspProviderContext)
// ---------------------------------------------------------------------------
 ...
 */
export function setLspProviderContext(projectId, projectRootPath) {
...
function modelToLspUri(model) {
  ...
  // cm://tab/{tabId}/{everything-after} — the "filename" may contain slashes
...
/**
 * Register all LSP-backed Monaco language providers for Python.
 *
 ...
 */
export function registerPythonLspProviders(monacoNs) {
  const disposables = [];

  // ── Completion ───────────────────────────────────────────────────────────
  disposables.push(monacoNs.languages.registerCompletionItemProvider('python', {
    triggerCharacters: ['.', '(', ',', ' ', '[', '"', "'"],
    ...
    async provideCompletionItems(model, position) {
      ...
      try {
        const result = await lspRequest(
          'python', _projectId, 'textDocument/completion',
          {
            textDocument: { uri },
  ...
  // ── Signature Help ────────────────────────────────────────────────────────
  disposables.push(monacoNs.languages.registerSignatureHelpProvider('python', {
    ...
    async provideSignatureHelp(model, position) {
      ...
      try {
        const result = await lspRequest(
          'python', _projectId, 'textDocument/signatureHelp',
          {
            textDocument: { uri },
            position: { line: position.lineNumber - 1, character: position.column - 1 },
          },
